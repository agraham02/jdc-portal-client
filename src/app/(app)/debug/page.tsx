...
export default function AuthDebugPage() {
  ...
  const runTest = async (testName: string, testFn: () => Promise<unknown>) => {
    ...
    try {
      ...
      addLog(`❌ ${testName} failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    addLog(`Debug Auth: ${process.env.NEXT_PUBLIC_DEBUG_AUTH || 'Not set'}`);
  }, []);
          ...
          <div className="mt-6 p-4 bg-gray-100 rounded">
            ...
            <pre className="text-sm">
              {JSON.stringify({
                ...
                DEBUG_AUTH: process.env.NEXT_PUBLIC_DEBUG_AUTH,
...
function ManualApiTest({ onLog }: { onLog: (message: string) => void }) {
  ...
  const runManualTest = async () => {
    ...
    try {
      let result;
      const bodyData = method !== 'GET' && body.trim() ? JSON.parse(body) : undefined;
      ...
      onLog(`❌ Manual test failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setLoading(false);
    }
  };
