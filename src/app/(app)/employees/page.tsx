...
import { CreateEmployeeDialog, EditEmployeeDialog } from "@/components/employees";

export default function EmployeesPage() {
    const [employees, setEmployees] = useState<EmployeeWithUser[]>([]);
    const [pendingEmployees, setPendingEmployees] = useState<EmployeeWithUser[]>([]);
    ...
    const loadEmployees = useCallback(async () => {
        try {
            setLoading(true);
            const response = await EmployeeService.getEmployees(currentPage, limit);
        ...
        <ProtectedRoute requiredRoles={RoleName.ADMIN}>
            <div className="container mx-auto p-6 space-y-6">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">
                                {employees.filter(emp => emp.userId.status === UserStatus.ACTIVE).length}
                            </div>
                        </CardContent>
                    </Card>
                </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>
                            {activeTab === "active" ? "Active Employees" : "Pending Employees"}
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        {loading ? (
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {(activeTab === "active" ? filteredEmployees : filteredPendingEmployees).map((employee) => (
                                        <TableRow key={employee._id}>
                                            <TableCell className="font-medium">
                                                {employee.userId.fullName || 
                                                 `${employee.userId.firstName || ''} ${employee.userId.lastName || ''}`.trim() ||
                                                 'N/A'}
                                            </TableCell>
                                            <TableCell>{employee.userId.email}</TableCell>
                                            <TableCell>{employee.jobTitle || 'N/A'}</TableCell>
                                            <TableCell>{employee.department || 'N/A'}</TableCell>
                                            <TableCell>{getStatusBadge(employee.userId.status)}</TableCell>
                                            <TableCell>
                                                {employee.hireDate 
                                            ...
                                            <TableCell className="text-right">
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        {employee.userId.status === UserStatus.PENDING && (
                                                            <DropdownMenuItem
